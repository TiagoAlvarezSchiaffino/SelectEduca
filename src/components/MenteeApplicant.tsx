import {
  ...
  export default function MenteeApplicant({ userId, showName, showContact } : {
    ...
    return !data ? <Loader /> : <Flex direction="column" gap={sectionSpacing}>
      <Heading size="md">{showName ? `${formatUserName(data.user.name, "formal")}` : ""}</Heading>
  
      {data.user.genre && <FieldRow name="" value={data.user.genre} />}
  
      {showContact && <>
        <FieldRow name="" value={data.user.wechat || ''} />
        <FieldRow name="" value={data.user.email} />
      </>}
  
      {!data.application ? "" : menteeApplicationFields.map(f => {
