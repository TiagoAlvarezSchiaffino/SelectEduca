...
/**
 * Return interview data for the mentees who are 1) in the "Current students" status and 
  ...
  .query(async ({ ctx }) => 
{
  ...
  const menteeStatus: MenteeStatus = "Current students";
  const mentees = await db.User.findAll({ 
    attributes: [...minUserAttributes, "genre", "menteeApplication"],
    where: { menteeStatus },
    include: [{
  ...
  return pendingMentees.map(m => ({
    ...
    profile: getMenteeProfile(`${ctx.baseUrl}/mentees/${m.id}`,
      m.genre, m.menteeApplication),
    averageDimensionalScores: getAverageDimensionalScores(m.interviews),
  }));
});

function getMenteeProfile(url: string, genre: string | null,
  ...
  interviews.filter(i => i.type == "MenteeInterview").map(i => {
    i.feedbacks.map(f => {
      ...
      dimensions && dimensions.map(d => {
        if (d.name && d.name !== "General comments" && d.score) {
