  ...
  .mutation(async ({ ctx, input }) => 
{
  ...
  if (!g) throw notFoundError("", input.groupId);

  checkPermissionForGroup(ctx.user, g);


  if (!apiEnv.hasTencentMeeting()) {
    console.log("tMeeting isn't configured. Fake a delay and return a mock meeting link.");
    await sleep(2000);
    return "/fakeMeeting";
  }
  ...
  return await sequelizeInstance.transaction(async (t) => {
    const ongoingMeeting = await db.OngoingMeetings.findOne({ 
      ...
      if (meetings.length >= apiEnv.TM_USER_IDS.length) {
        await emailRoleIgnoreError("SystemAlertSubscriber", "", 
          `${apiEnv.TM_USER_IDS.length}${ctx.baseUrl}/groups/${input.groupId}`, ctx.baseUrl);
        return null;
      }
