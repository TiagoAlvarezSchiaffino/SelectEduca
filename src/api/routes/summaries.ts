...
const AI_MINUTES_SUMMARY_KEY = "Smart minutes";
  ...
  .query(async ({ ctx, input: transcriptId }) => 
{
  ...
  if (!t) throw notFoundError("Meeting Transcription", transcriptId);
...
function formatSpeakerStats(speakerStats : SpeakerStats) : string {
  ...
  return "*Speech duration statistics (minutes)* : " + speakerStats.map(s =>
